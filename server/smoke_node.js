(async () => {
  ...
  function log(title, obj){
    console.log('----', title, '----')
    try{ console.log(JSON.stringify(obj, null, 2)) } catch(_e){ console.log(obj) }
  }
  ...
  try{ json = JSON.parse(json) } catch (_e){}
    ...
    json = await res.text();
  try{ json = JSON.parse(json) } catch (_e){ /* ignore JSON parse errors */ }
  ...
  const refreshRes = await fetch(base + '/auth/refresh', { method: 'POST', headers: { 'content-type':'application/json', cookie: refreshCookie || '' } })
  let refreshJson = await refreshRes.text();
  try{ refreshJson = JSON.parse(refreshJson) } catch (_e){ /* ignore JSON parse errors */ }
  ...
  const refreshRes2 = await fetch(base + '/auth/refresh', { method: 'POST', headers: { 'content-type':'application/json', cookie: refreshCookie || '' } })
  let refreshJson2 = await refreshRes2.text(); try{ refreshJson2 = JSON.parse(refreshJson2) } catch (_e){ /* ignore JSON parse errors */ }
  log('REFRESH-AGAIN-OLD', { status: refreshRes2.status, body: refreshJson2 })

    // 6) Logout using cookie
  const logoutRes = await fetch(base + '/auth/logout', { method: 'POST', headers: { cookie: refreshCookie || '' } })
  let logoutJson = await logoutRes.text(); try{ logoutJson = JSON.parse(logoutJson) } catch (_e){ /* ignore JSON parse errors */ }
  log('LOGOUT', { status: logoutRes.status, body: logoutJson })
    ...
    console.error('ERROR', _err?.message || _err);
    console.error(_err);
  }
})();
